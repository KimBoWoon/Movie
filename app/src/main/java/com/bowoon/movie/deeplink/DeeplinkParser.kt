...
fun parseDeeplink(uri: Uri?): List<NavKey> = uri?.let { uri ->
    ...
    DeeplinkAction.entries.find { it.value == path }?.let { deeplinkAction ->
        when (deeplinkAction) {
            ...
            DeeplinkAction.SEARCH_TO_MOVIE -> createDeeplinkStack(command = SearchToMovie(query = query["query"] ?: "", searchType = query["searchType"] ?: "movie", id = query["id"]?.toIntOrNull() ?: -1))
