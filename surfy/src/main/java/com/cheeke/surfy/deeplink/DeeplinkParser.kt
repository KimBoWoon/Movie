...
fun parseDeeplink(uri: Uri?): List<NavKey> = uri?.let { uri ->
    ...
    DeeplinkAction.entries.find { it.value == path }?.let { deeplinkAction ->
        when (deeplinkAction) {
            ...
            DeeplinkAction.GO_TO_FAVORITE -> createDeeplinkStack(command = DeeplinkCommand.GoToFavorite(
                tabIndex = query["tabIndex"]?.toIntOrNull() ?: 0
            )
            )
            DeeplinkAction.GO_TO_SETTING -> createDeeplinkStack(command = DeeplinkCommand.GoToSetting)
            DeeplinkAction.OPEN_FAVORITE_PERSON -> createDeeplinkStack(command = DeeplinkCommand.OpenFavoritePeople(
                id = query["peopleId"]?.toIntOrNull() ?: -1
            )
            )
            DeeplinkAction.OPEN_FAVORITE_MOVIE -> createDeeplinkStack(command = DeeplinkCommand.OpenFavoriteMovie(
                id = query["id"]?.toIntOrNull() ?: -1
            )
            )
            DeeplinkAction.GO_TO_MOVIE -> createDeeplinkStack(command = DeeplinkCommand.GoToMovie(
                id = query["id"]?.toIntOrNull() ?: -1
            )
            )
            DeeplinkAction.GO_TO_PEOPLE -> createDeeplinkStack(command = DeeplinkCommand.GoToPeople(
                id = query["id"]?.toIntOrNull() ?: -1
            )
            )
            DeeplinkAction.GO_TO_SERIES -> createDeeplinkStack(command = DeeplinkCommand.GoToSeries(
                id = query["id"]?.toIntOrNull() ?: -1
            )
            )
            DeeplinkAction.GO_TO_SEARCH -> createDeeplinkStack(command = DeeplinkCommand.GoToSearch(
                query = query["query"] ?: "",
                searchType = query["searchType"] ?: "surfy"
            )
            )
            DeeplinkAction.SEARCH_TO_MOVIE -> createDeeplinkStack(command = DeeplinkCommand.SearchToMovie(
                query = query["query"] ?: "",
                searchType = query["searchType"] ?: "surfy",
                id = query["id"]?.toIntOrNull() ?: -1
            )
            )
            DeeplinkAction.SEARCH_TO_PEOPLE -> createDeeplinkStack(command = DeeplinkCommand.SearchToPeople(
                query = query["query"] ?: "",
                searchType = query["searchType"] ?: "people",
                id = query["id"]?.toIntOrNull() ?: -1
            )
            )
            DeeplinkAction.SEARCH_TO_SERIES -> createDeeplinkStack(command = DeeplinkCommand.SearchToSeries(
                query = query["query"] ?: "",
...
private fun createDeeplinkStack(command: DeeplinkCommand): List<NavKey> = when (command) {
    ...
    is DeeplinkCommand.OpenFavoritePeople -> listOf(FavoriteNavKey(tab = 0), PeopleNavKey(id = command.id))
    is DeeplinkCommand.OpenFavoriteMovie -> listOf(FavoriteNavKey(tab = 1), MovieNavKey(id = command.id))
